...
export function useTouchReorder(options:Options){
 ...
 useEffect(()=>{
  ...
  let preview:ReturnType<typeof createReorderPreview>|null=null;
  ...
  const stop=(save=false)=>{
   clearTimeout(pending);cancelAnimationFrame(frame);preview?.remove();preview=null;const d=drag;drag=null;
   if(!d)return;
  ...
  const start=(event:TouchEvent)=>{
   ...
   pending=setTimeout(()=>{if(!drag)return;drag.active=true;preview=createReorderPreview(drag.row,drag.x,drag.y);document.body.classList.add('touch-reordering');window.getSelection()?.removeAllRanges();latest.current.onStart(kind,id);frame=requestAnimationFrame(tick)},400);
  };
